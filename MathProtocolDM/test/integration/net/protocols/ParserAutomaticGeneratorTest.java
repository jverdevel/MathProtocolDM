 ...
 * Assumes network and transport protocol are txt-ip and stupid-trans. Allows
 * for changing for any application protocol
 * 
 * @author VV
 *
...
public abstract class ParserAutomaticGeneratorTest<T extends IProcessedApplicationPacket> {
	 ...
	 * @param numComms number of communications (including a request and response
	 *                 each) generated
	 */
	protected final void doTest(long seed, int numComms) {
		...
		// constructing messages, and might end up spending a lot more memory running
		// toString on the list than what is actually spend on the test
	...
	private Comm<T> autogenerateComm(Random random) {
		...
		Msg<T> msgRequest = new Msg<>(new AddressPort(originAddress, originPort),
				new AddressPort(destinationAddress, destinationPort), opsRequest);
		Msg<T> msgResponse = new Msg<>(new AddressPort(destinationAddress, destinationPort),
				new AddressPort(originAddress, originPort), opsResponse);
	 ...
	 * Generates a test case
	 * 
	 * @param comms  communications to use. Mixes up responses and requests in a way
	...
	private SnapshotAndExpectedComms generateTestCase(List<Comm<T>> comms, Random random) {
		...
		while (!unprocessedComms.isEmpty() || !halfProcessedComms.isEmpty()) {
			if (halfProcessedComms.isEmpty()) {
				...
				// Very much an edge case, but if the half processed top happens to be a valid
				// response to the unprocessed top, we need to do the half processed before to
				// avoid two consecutive equal requests without a response from the same client
				// to the same server, which looks protocol-incompatible
				this.processSnapshotResponse(snapshot, halfProcessedComms, processedComms);
			} else {
				// solve at random
	...
	/**
	 * Adds a request to the snapshot and marks it as half-processed (only response
	 * left to send)
	 * 
	 * @param snapshot           snapshot
	 * @param unprocessedComms   queue of communications that haven't been sent
	 * @param halfProcessedComms queue of communications whose request has been
	 *                           sent, but not the response
	 */
	private void processSnapshotRequest(StringBuilder snapshot, Queue<Comm<T>> unprocessedComms,
			Queue<Comm<T>> halfProcessedComms) {
	...
	/**
	 * Adds a response to the snapshot and marks it as half-processed (only response
	 * left to send)
	 * 
	 * @param snapshot           snapshot
	 ...
	 */
	private void processSnapshotResponse(StringBuilder snapshot, Queue<Comm<T>> halfProcessedComms,
			List<Comm<T>> processedComms) {
	...
	/**
	 * Calculates if the first members in the queue could be misunderstood as a
	 * reply to the other
	 * 
	 * @param unprocessedComms   queue of communications that haven't been sent
	 * @param halfProcessedComms queue of communications whose request has been
	...
	private boolean isNextValidResponse(Queue<Comm<T>> unprocessedComms, Queue<Comm<T>> halfProcessedComms) {
		...
		return unprocessed.getRequest().getOriginAddress().equals(halfProcessed.getResponse().getDestinationAddress())
				&& unprocessed.getResponse().getOriginAddress()
						.equals(halfProcessed.getRequest().getDestinationAddress());
	}
	 ...
	 * @param random             randomness generator
	 */
	private void processSnapshotRandom(StringBuilder snapshot, Queue<Comm<T>> unprocessedComms,
			Queue<Comm<T>> halfProcessedComms, List<Comm<T>> processedComms, Random random) {
