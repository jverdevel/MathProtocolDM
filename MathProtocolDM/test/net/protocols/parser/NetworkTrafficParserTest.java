...
public class NetworkTrafficParserTest {
	...
	public void testValidPairing() {
		...
		IPortCompatibleTransportProtocolParser transportParser = Mockito.mock(IPortCompatibleTransportProtocolParser.class);
		...
		NetworkTrafficParser parser = new NetworkTrafficParser<>(networkParser, transportParser, applicationParser, generator);
	...
	private void mockPairedPackages(IIpBasedNetworkProtocolParser parser, IPortCompatibleTransportProtocolParser transportParser, ITrafficSnapshot snapshot) {
	...
	private void mockNonPairedPackages(IIpBasedNetworkProtocolParser parser, IPortCompatibleTransportProtocolParser transportParser, ITrafficSnapshot snapshot) {
		List<IpBasedNetworkPacket> packets = new ArrayList<>();
		packets.add(this.mockPacket(transportParser, ORIGIN_IP_1, ORIGIN_IP_2, ORIGIN_PORT_1, ORIGIN_PORT_2));
		packets.add(this.mockPacket(transportParser, ORIGIN_IP_3, ORIGIN_IP_4, ORIGIN_PORT_3, ORIGIN_PORT_4));
		Mockito.when(parser.processPackages(Mockito.any())).thenReturn(packets);		
	...
	private IpBasedNetworkPacket mockPacket(IPortCompatibleTransportProtocolParser mockTransport, String ipOrigin, String ipDestination, String portOrigin, String portDestination) {
		ITrafficSnapshot snapshot = Mockito.mock(ITrafficSnapshot.class);
		...
		PortCompatibleTransportPacket packetTransport = new PortCompatibleTransportPacket(portOrigin, portDestination, mockSnapshotTransport);
