...
/**
 * Test class to provide mocks for a traffic snapshot that substrings and
 * creates other snapshots properly without worring about details
 * 
 * @author Javier Verde
 *
 */
public class ITrafficSnapshotTestUtil {
	...
	public static ITrafficSnapshot createSnapshot(String text) {
		...
		Mockito.when(mockSnapshot.getSnapshotFragment(Mockito.anyInt(), Mockito.anyInt()))
				.thenAnswer(new AnswerSubSnapshot(text));
		return mockSnapshot;
	}
