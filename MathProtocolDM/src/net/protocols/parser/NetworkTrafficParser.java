...
public class NetworkTrafficParser<T extends IProcessedApplicationPacket> {
	 ...
	 * @param raw byte array. Byte is assumed to be gotten from the getBytes() string method
	...
	public List<Comm<T>> crunch(byte[] raw) throws ParseSnapshotException{
		...
		//VV: now, for each network packet, we find out the associate transport packet information. 
		Stream<AddressDataWithApplicationSnapshot> applicationSnapshots = this.getAddressDataFromNetwork(networkPackets);
	...
	private List<IpBasedNetworkPacket> parseNetworkPackets(ITrafficSnapshot snapshot) throws ParseSnapshotException{
		...
		while(currentSnapshot.getLength()!=0) {
			...
			currentSnapshot = currentSnapshot.getSnapshotFragment(newPackage.getTotalLength(), currentSnapshot.getLength());
		}
		return networkPackages;
	}
	
	/**
	 * Extracts transport-layer data from packet and readies full IP and port based address data from network packets
	 * @param packets network packets
	 * @return stream of addresses with remaining application data
	 */
	private Stream<AddressDataWithApplicationSnapshot> getAddressDataFromNetwork(Collection<IpBasedNetworkPacket> packets){
	 ...
	 * @throws ParseSnapshotException
	 */
	private AddressDataWithApplicationSnapshot getAddressDataFromNetwork(IpBasedNetworkPacket packet) throws ParseSnapshotException {
		PortCompatibleTransportPacket transportPacket = this.transportParser.processPackage(packet.getTransportLayerData());
		AddressPort addressOrigin = new AddressPort(packet.getOriginIp(), transportPacket.getOriginPort());
		AddressPort addressDestination = new AddressPort(packet.getDestinationIp(), transportPacket.getDestinationPort());
		return new AddressDataWithApplicationSnapshot(addressOrigin, addressDestination, transportPacket.getApplicationLayerData());
	}
	
	/**
	 * Pairs communication data in coupled request-response pairs. Expected all communication data to be paired
	 ...
	 * @throws ParseSnapshotException 
	 */
	private Stream<PairedCommuncationsData> pairCommunicationsData(ITrafficSnapshot mainSnapshot, Stream<AddressDataWithApplicationSnapshot> data) throws ParseSnapshotException{
		//Reading how the protocol works, this makes an assumption: 
		//1) Client won't make another request to the same server before first one is answered. Since application doesn't use any identification for the OP, and trusts on the client address, this seems to be the case
		...
		//If map is not empty, there's unpaired comms. that should not happen. If we wanted to process these, we could just fill another list, extract application data with the same methods, and return it in an object different that Comm. 
		if(!dataMap.isEmpty()) {
			AddressDataWithApplicationSnapshot unpaired = dataMap.get(dataMap.keySet().iterator().next());
			throw new ParseSnapshotException(unpaired.getRemainingApplication(), NETWORK_LAYER, 0, "Unpaired communication");
		}
	 ...
	 */
	private void pairCommunication(Map<String, AddressDataWithApplicationSnapshot> currentStatus, AddressDataWithApplicationSnapshot data, List<PairedCommuncationsData> result ) {
	 ...
	 * @throws ParseSnapshotException
	 */
	private List<Comm<T>> getCommsFromPairedData(Stream<PairedCommuncationsData> validPairs) throws ParseSnapshotException{
	...
	private static class AddressDataWithApplicationSnapshot{
		 ...
		 */
		public AddressDataWithApplicationSnapshot(AddressPort addressDataOrigin, AddressPort addressDataDestination, ITrafficSnapshot remainingApplication) {
			super();
